...
abstract class Parse {
  ...
  static Future<MultipartFile> toMultipartFile(Asset asset) async {
    int width = asset.originalWidth == null
        ? AppConfig.instance.values.medias?.post?.image?.maxHeight ?? 1920
        : asset.originalWidth!;
    int height = asset.originalHeight == null
        ? AppConfig.instance.values.medias?.post?.image?.maxWidth ?? 1080
        : ((width * asset.originalHeight!) / asset.originalWidth!).round();

    ByteData byteData = await asset.getThumbByteData(width, height);
