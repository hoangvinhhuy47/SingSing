...
class SettingWalletBloc extends Bloc<SettingWalletEvent, SettingWalletState> {
  ...
  SettingWalletBloc({
    ...
    on<OnWalletDeletingEvent>(_mapOnDeletingEventToState);
  }
  ...
  Future<void> _mapOnDeletingEventToState(
      OnWalletDeletingEvent event, Emitter<SettingWalletState> emit) async {
    emit(SettingWalletStateWalletDeleting());
    final result = await DbManager.instance.deleteWallet(wallet);
    if (result == 0) {
      emit(const SettingWalletStateDeleteWalletDone(
          message: 'Delete wallet failed', isSuccess: false));
    } else {
      emit(const SettingWalletStateDeleteWalletDone(
          message: "Delete wallet successfully", isSuccess: true));
    }
    emit(SettingWalletStateLoaded());
  }
}