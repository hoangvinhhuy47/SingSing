...
class _WalletDetailScreenState extends State<WalletDetailScreen>
  ...
  void initState() {
    ...
    _customTokenListener = App.instance.eventBus
        .on<EventBusAddCustomTokenSuccess>()
        .listen((event) {
      _walletDetailTokenBloc.add(WalletDetailTokenEventReload());
    });
    _transferNftListener = App.instance.eventBus
        .on<EventBusTransferNftSuccessful>()
        .listen((event) {
      _walletDetailNftBloc.add(WalletDetailNftEventReload());
    });
    _transferTokenListener = App.instance.eventBus
        .on<EventBusTransferTokenSuccessful>()
        .listen((event) {
      _walletDetailTokenBloc.add(WalletDetailTokenEventReload());
    });
    _changeWalletNameListener = App.instance.eventBus
        .on<EventBusChangeWalletNameSuccessful>()
        .listen((event) {
      _walletDetailTokenBloc
          .add(ChangeWalletNameSuccessful(event.walletId, event.name));
    });
  ...
  List<Widget> getTabBarWidgetItem(WalletDetailState state) {
    return [
      _buildTokenView(),
      if (AppConfig.instance.values.nftEnable) _buildNftView(),
    ];
  }
  ...
  _getDollarPrice(String? balance, String? symbol) {
    ...
    final price = App.instance.priceTable[symbol.toUpperCase()];
    if (price == null) {
      return '';
    }
  ...
  _getBtcPrice(String? balance, String? symbol) {
    ...
    //btc
    final btcPrice = App.instance.priceTable['BTC'];
    if (btcPrice == null) {
      return '';
    }
      ...
      child: TextButton.icon(
        onPressed: () => {
          Navigator.pushNamed(context, Routes.selectTokenScreen, arguments: {
            ...
            'price_table': App.instance.priceTable,
  ...
  Future<void> onRefreshNft() async {
    if (!_walletDetailNftBloc.isClosed) {
      _walletDetailNftBloc.add(WalletDetailNftEventReload());
    }
