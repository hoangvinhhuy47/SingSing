...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  void initState() {
    ...
    _changeUserInfoListener = App.instance.eventBus
        .on<EventBusChangeUserInfoSuccessful>()
        .listen((event) {
      _userProfileScreenBloc
          .add(ChangeUserInfoSuccessfulEvent(email: event.email));
    });
  ...
  Widget _buildBody(ProfileScreenState state) {
    ...
    return Column(
      children: [
        Expanded(
            child: LoginRegisterView(
          onLoginPressed: () {
            _userProfileScreenBloc.rootBloc.add(ShowLoginPopupEvent());
          },
          onRegisterPressed: () {
            _userProfileScreenBloc.rootBloc.add(ShowRegistrationPopupEvent());
          },
        )),
        _buildAppVersion(),
      ],
  ...
  _buildAppVersion() {
    return Text(
        ...
        style: s(context, fontSize: 16, color: ColorUtil.mainGrey));
  }
              ...
              children: [
                SizedBox(
                  ...
                  child: ImageUtil.loadAssetsImage(
                      fileName: icon, height: 18, color: ColorUtil.lightGrey),
                ),
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.only(
            ...
            child: Row(
              children: [
                SizedBox(
                  ...
                  height: 12,
                  child: ImageUtil.loadAssetsImage(
                      fileName: AppLocalization.instance.currentLangCode ==
                              LangCode.en
                          ? 'en.png'
                          : 'vn.png',
                      height: 12,
                ...
                ImageUtil.loadAssetsImage(
                    fileName: 'ic_arrow_right.svg', color: ColorUtil.lightGrey),
              ],
            ),
          ),
          onTap: () {
        ...
        children: [
          SizedBox(
            ...
            child: ImageUtil.loadNetWorkImage(
                url: _userProfileScreenBloc.nftUserProfile?.cover ?? '',
                height: 180,
                fit: BoxFit.fill,
    ...
    return TextButton(
      ...
      style: ButtonStyle(
        ...
        backgroundColor: MaterialStateProperty.all<Color>(
            const Color(0xFF161B2E).withOpacity(0.5)),
      ),
      child: SizedBox(
        width: 75,
        height: 32,
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          ImageUtil.loadAssetsImage(
              fileName: 'ic_edit.svg', width: 16, fit: BoxFit.contain),
          const SizedBox(
            width: 8,
          ),
          Text(
  ...
  _buildUsernameAndAvatar() {
    return Row(
      children: [
        const SizedBox(
          width: MyStyles.horizontalMargin * 2,
        ),
        _buildAvatar(),
        const SizedBox(
          width: MyStyles.horizontalMargin,
        ),
        Expanded(
          child: Wrap(
            children: [_buildUsername()],
          ),
        ),
        const SizedBox(
          width: MyStyles.horizontalMargin,
  ...
  Widget _avatar() {
    if (_userProfileScreenBloc.fileUploadAvatar != null) {
      return Image.file(_userProfileScreenBloc.fileUploadAvatar!,
          height: avatarHeight, width: avatarHeight, fit: BoxFit.cover);
    } else {
      return ImageUtil.loadNetWorkImage(
          url: App.instance.user?.avatar ?? '',
          height: avatarHeight,
          width: avatarHeight,
  ...
  _buildAvatarEditButton() {
    return Container(
        ...
        child: TextButton(
          onPressed: _onAvatarEditButtonPressed,
          style: ButtonStyle(
            overlayColor:
                MaterialStateProperty.all<Color>(Colors.white.withOpacity(0.3)),
            padding: MaterialStateProperty.all<EdgeInsets>(EdgeInsets.zero),
          ...
          child: SizedBox(
            ...
            height: 12,
            child: ImageUtil.loadAssetsImage(
                fileName: 'ic_edit.svg', width: 12, fit: BoxFit.scaleDown),
          ),
        ));
  }
      ...
      try {
        ...
        if (pickedFile != null) {
          ...
          if (sizeInMb > 2) {
            showSnackBarError(
                context: context,
                message: l('File size is less than or equal to 2MB'));
            return;
          }
          _userProfileScreenBloc.add(ProfileScreenEventUploadAvatar(
              file: imageFile, mimeType: pickedFile.mimeType ?? ''));
          return;
        } else {
    ...
    if (await Permission.photos.request().isGranted) {
      try {
        ...
        if (pickedFile != null) {
          ...
          _userProfileScreenBloc.add(ProfileScreenEventUploadCover(
              file: imageFile, mimeType: pickedFile.mimeType ?? ''));
          return;
        }
      } catch (e) {
