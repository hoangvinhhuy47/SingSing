...
import 'package:sing_app/utils/app_lock_util.dart';
...
class _SendNftScreenState extends State<SendNftScreen> {
  ...
  Widget _buildBody() {
    double imageWidth =
        MediaQuery.of(context).size.width - MyStyles.horizontalMargin * 2;
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _sendNftScreenBloc.nft.externalData?.name ?? '',
    ...
    return TextFormField(
      ...
      validator: (value) {
        if (value == null || value.isEmpty) {
          return l('The receiving address field cannot be left blank');
        }
        if (!StringExtension.validateWalletAddress(value)) {
          return l('Incorrect receiving address');
        }
        return null;
      },
  ...
  _onScanButtonPressed() async {
    ...
    if (await Permission.camera.request().isGranted) {
      final result =
          await Navigator.pushNamed(context, Routes.sendTokenScanQrCodeScreen);
  ...
  _onPressedSend() {
    ...
    AppLockUtil.checkPassCode(
      ...
      function: () {
        ...
        _eventSendNft(address);
      },
    );
  }

  _eventSendNft(String address) {
