...
class Oauth2Manager {
  ...
  String getUrlAuthorize() {
    ...
    String os = Platform.isIOS ? 'os=ios' : 'os=android';

    String urlAuthorize = '${AppConfig.instance.values.authUrl}${Oauth2Config.authorizeUrl}?$responseType&$clientId&$redirectUrl&$scope&$state&$codeChallenge&$codeChallengeMethod&$os';
  ...
  String getUrlRegistration() {
    ...
    String urlRegistraions = '${AppConfig.instance.values.authUrl}${Oauth2Config.registrationsUrl}?$responseType&$clientId&$redirectUrl&$scope&$state&$codeChallenge&$codeChallengeMethod&$os';
  ...
  Future<UserProfile?> getUserApp() async {
    if (userAppInfo != null) {
      return userAppInfo;
    }
    final userJson = await SecureStorageUtil.shared.readData(userAppStorageKey);
    if (userJson != null) {
      userAppInfo = UserProfile.fromJson(jsonDecode(userJson));
    }
    return userAppInfo;
  }

  storeUserApp(UserProfile user) async {
    userAppInfo = user;
    final String userJson = jsonEncode(user);
    SecureStorageUtil.shared.writeData(userAppStorageKey, userJson);
  }

  _clearUser() async {
    userAppInfo = null;
    SecureStorageUtil.shared.deleteKey(userAppStorageKey);
  }
