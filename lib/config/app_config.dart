...
import '../data/models/app_config/medias_config.dart';
...
const String appStagingAndroidPackageName = 'net.singsing.app.stag';
const String appDevelopmentAndroidPackageName = 'net.singsing.app.dev';

const String appStagingIOSPackageName = 'net.singsing.appstag';
const String appDevelopmentIOSPackageName = 'net.singsing.appdev';
    ...
    switch (this) {
        ...
        return FlavorValues(
          ...
          supportChains: {
            secretTypeEth: SupportChain(
              ...
              name: 'Ethereum',
              secretType: secretTypeEth,
              moralisChainName: 'eth testnet',
            ),
            secretTypeBsc: SupportChain(
              ...
              name: 'Smart Chain',
              secretType: secretTypeBsc,
              moralisChainName: 'bsc testnet',
            ),
          },
          rpcUrls: [
          ...
          ethRpcUrls: [
            'https://rinkeby.infura.io/v3/919f8cd33cac4251837cd8fea72a6aa0',
          ],
          singApiUrl: 'https://mobile-stag.singsing.net',
          nftEnable: true,
        );
      ...
      default:
        return FlavorValues(
          ...
          ethRpcUrls: [
            'https://mainnet.infura.io/v3/919f8cd33cac4251837cd8fea72a6aa0',
          ],
          singSingContractAddress: '',
...
class FlavorValues {
  ...
  final List<String> ethRpcUrls;
  final String ssoSecretKey;
  List<String> rpcUrls = [];
  List<String> reportContents = [];
  List<String> graphBlacklistDomains = [];
  MediasSsConfig? medias;
  ...
  void merge(Map<String, dynamic> json) {
    ...
    if (json.containsKey('rpc_urls')) {
      try {
        List<dynamic> jsonRpcUrls =
            json.containsKey('rpc_urls') ? json['rpc_urls'] : [];
        rpcUrls = [];
    ...
    if (json.containsKey('report_contents')) {
      try {
        List<dynamic> jsonReportContents =
            json.containsKey('report_contents') ? json['report_contents'] : [];
        reportContents = [];
        for (dynamic url in jsonReportContents) {
          reportContents.add(url);
        }
      } catch (_) {}
    }
    if (json.containsKey('medias')) {
      try {
        MediasSsConfig mediaSSConfig = MediasSsConfig.fromJson(json['medias']);
        medias = mediaSSConfig;
      } catch (e) {
        log('MediasSsConfig e ${e.toString()}');
      }
    }

    if (json.containsKey('graph_blacklist_domains')) {
      try {
        List<dynamic> jsonGraphBlacklistDomains =
        json.containsKey('graph_blacklist_domains') ? json['graph_blacklist_domains'] : [];
        graphBlacklistDomains = [];
        for (dynamic url in jsonGraphBlacklistDomains) {
          graphBlacklistDomains.add(url);
        }
      } catch (_) {}
    }
...
class AppConfig {
  ...
  bool isSingSingContract(String contract) {
    return contract.equalsIgnoreCase(
        _instance!.flavor.getValues().singSingContractAddress);
  }
}
