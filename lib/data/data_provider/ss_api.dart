...
import '../../constants/api_key_param.dart';
...
class SsAPI {
  ...
  Map<String, String> get _defaultHeaders {
    return {
      'x-lang': AppLocalization.instance.locale.languageCode,
    ...
    try {
      ...
      if (exception.response != null) {
        if (exception.response?.data is! String) {
          if(isUseAccessToken && checkTokenExpired(exception.response?.data)) {
            return null;
          }
  ...
  bool checkTokenExpired(dynamic json) {
    final code = json['code'];
    if (code != null && code == 'invalid_or_expired_auth_token') {
      _rootBloc.add(AccessTokenExpired());
      return true;
    }

    if(json[ApiKeyParam.error] != null){
      SsError? error = SsError.fromJson(json[ApiKeyParam.error]);
      if(error.code == 401) {
        _rootBloc.add(AccessTokenExpired());
        return true;
      }
  ...
  Future<dynamic> put({
    ...
    try {
      ...
      LoggerUtil.info(
          'PUT $url response headers: ${responseData.headers} - data: ${responseData.data}',
          tag: tag);
      return responseData.data;
    } on DioError catch (exception) {
      LoggerUtil.error('BaseAPI response error: $url - ${exception.response}');
  ...
  Future<dynamic> post({
    ...
    try {
      ...
      await _setHeaders(headers, isUseAccessToken);
      responseData = await _dio.post(url, data: params ?? bodyParams ?? {});
      // LoggerUtil.info('$url response: $responseData', tag: tag);
      return responseData.data;
    } on DioError catch (exception) {
      LoggerUtil.error('BaseAPI response error: $url - ${exception.response}');
