...
class WalletApiProvider {
  ...
  Future<DefaultSsResponse< UserProfile >> getUserProfile() async {
    try {
      final Map responseJson = await _baseAPI.request(
        manager: ApiManager(ApiType.userProfile),
        isUseAccessToken: true
      );
      LoggerUtil.info('getUserProfile responseJson: $responseJson}');
      final response = DefaultSsResponse.fromMap(responseJson);
      LoggerUtil.info('getUserProfile response: ${response.toJson().toString()}');
      if (response.success && response.data != null) {
      ...
      return DefaultSsResponse<UserProfile>.withError(
        SsError(message: exception.toString()),
      );
    }
  }
